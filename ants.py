...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if exit: #if Place has an exit
            exit.entrance = self #exit's entrance is set to that Place
        # END Problem 2
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.can_contain(insect):
                    ...
                    self.ant = insect   #container ant is the main ant
                else:
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3
    min_range = 0
    max_range = float("inf") #original ThrowerAnt has no minimum or maximum range
    ...
    def nearest_bee(self, beehive):
        ...
        min_dist = self.min_range
        distance = 0
        place = self.place
        while place != beehive:
            if distance > max_dist:
                return None
            if random_or_none(place.bees) and distance >= min_dist:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        if self.digesting > 0: #if digesting, decrement its digesting counter.
            self.digesting -= 1
        else:
            random_bee = random_or_none(self.place.bees) #eat a random Bee in its place
            if random_bee:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        for bee in list(self.place.bees): #reduce the armor of all Bees in the same place as the NinjaAnt by its damage attribute.
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.first_queen:
            ...
            curr_spot=self.place.exit
            while curr_spot:
                ant_in_spot=curr_spot.ant
                if ant_in_spot and ant_in_spot.name!='Queen':
                    if ant_in_spot.is_container:
                        contained_ant=ant_in_spot.contained_ant
                        if ant_in_spot not in self.doubled_ants:
                            ant_in_spot.damage = (ant_in_spot.damage)*2
                            self.doubled_ants+=[ant_in_spot]
                        if contained_ant and contained_ant not in self.doubled_ants and contained_ant.name!='Queen':
                                contained_ant.damage = (contained_ant.damage)*2
                                self.doubled_ants += [contained_ant]
                    elif ant_in_spot not in self.doubled_ants:
                            ant_in_spot.damage=(ant_in_spot.damage)*2
                            self.doubled_ants+=[ant_in_spot]
                curr_spot=curr_spot.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if not self.first_queen and self.armor<=0:
            self.place.remove_insect(self)
            self.death_callback()
        elif self.first_queen and self.armor<=0:
            bees_win()
        # END Problem 13
